function [G, Adj]=lambda_knn_abs(Points,k,f1,f2,lambdaMethod)
%   Syntax:
%       [G, Adj]=lambda_knn_abs(Point,k,f1,f2);
%       [G, Adj]=lambda_knn_abs(Point,k,f1,f2,lambdaMethod);
%   Description:
...
for i=1:N
    ...
    for j=1:N
        if(i==j)
            ...
            V(i,j)=dist(i,j)+lamda1*sign(rank(1,j)-kL+eps);
        end
    end         
end
...
for i=1:count     
    ...
    delta=Table(i,3)+lamda2*(abs(avg_deg+(1/N)-kG)-abs(avg_deg-kG));
