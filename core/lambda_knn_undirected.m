function [G, Adj]=lambda_knn_undirected(Points,k,f1,f2,lambdaMethod)
% LAMBDA_DIR_UNDIRECTED this method computes a directed graph using the
% combines lambda-KNN method.
%   Syntax:
%       [G, Adj]=lambda_knn_undirected(Point,k,f1,f2);
%       [G, Adj]=lambda_knn_undirected(Point,k,f1,f2,lambdaMethod);
%   Description:
...
for i=1:count     
    ...
    G_new(from,to)=Table(i,4);   
    G_new(to,from)=Table(i,4);   
    Adj_new(from,to)=1; 
    Adj_new(to,from)=1; 
    
    delta=Table(i,3)+lamda2*((avg_deg+(2/N)-kG)^2-(avg_deg-kG)^2);    
